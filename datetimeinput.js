...
(function(factory) {
  ...
  'use strict';
  angular.module('ui.dateTimeInput', []).value('dateTimeInputConfig').directive('dateTimeInput', [dateTimeInputDirective]);
  ...
  function dateTimeInputDirective() {
    ...
    function linkFunction(scope, element, attrs, controller) {
      ...
      if (angular.isDefined(attrs.viewTz) && (!angular.isString(attrs.viewTz) || attrs.viewTz.length === 0)) {
        throw 'view TZ must be "UTC" or "LOCAL"';
      }
      ...
      function unique(value, index, self) {
        return ['Date', 'moment', 'milliseconds', undefined].indexOf(value) === -1 && self.indexOf(value) === index;
      }
      ...
      function formatter(modelValue) {
			//console.log ("inside formatter; modelValue :" + modelValue);
        ...
        if (angular.isDate(modelValue)) {
        	...
        	if (viewTz == "LOCAL") {
        		var tempValue = moment(modelValue); //when printed, I think it automatically prints valueof
        		tempValue.local();
        		return moment(tempValue).format(displayFormat);       // Display Local Timezone - added by AT
        	} 
        	else {
        		return moment.utc(modelValue).format(displayFormat); 	 // Display UTC
        	}       	         	
        } else if (angular.isDefined(modelValue)) {
        	if (viewTz == "LOCAL") {
        		var tempValue = moment.utc(modelValue); //when printed, I think it automatically prints valueof
        		tempValue.local();
        		return moment(tempValue, inputFormats, moment.locale(), dateParseStrict).format(displayFormat);
	       	}
        	else {
